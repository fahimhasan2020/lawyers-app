...
import { serviceCreateApi, serviceEditApi } from '../../data/api/ServiceCreate';
import { showMessage } from 'react-native-flash-message';
import { fireMessage } from '../../utility/flashMessageFire';
const ServiceEditModal = ({showServiceModal=false,setShowServiceModal=()=>{},datas = null}) => {
  const [categories,setCategories] = useState([]);
  const userId = useSelector(state=>state.auth.id);
  const [name,setName] = useState("");
  const [details,setDetails] = useState("");
  const [serviceId,setServiceId] = useState("");
  const [currentMaximumRange,setCurrentMaximumRange] = useState(120);
  ...
  const saveService = async()=>{
    const datas = {
      ...
      minimum_range:100,
      maximum_range:currentMaximumRange
    };
    console.log(datas);
    const createOne = await serviceEditApi(datas);

    if(createOne?.success){
      fireMessage("Service updated successfully","success");
    }else{
      fireMessage("Failed to update service","danger");
    }
    setName("");
    setDetails("");
    setCurrentValue(0);
  ...
  useEffect(()=>{
    setServiceId(datas?.id);
    setName(datas?.name);
    setDetails(datas?.details);
    setCurrentMaximumRange(parseInt(datas?.maximum_range));
    fetchDatas();
  },[datas])
  return (
    <View>
        ...
        >
          ...
          />
            ...
            maxLength={100}
          />
          <Text marginT-s3>সর্বনিম্ন পরিষেবা ফি - ( {currentMaximumRange} টাকা )</Text>
          <Slider
            value={currentMaximumRange}
            minimumValue={100}
            maximumValue={10000}
            minimumTrackTintColor={Colors.yellow20}
            thumbTintColor={Colors.yellow20}
            onValueChange={(value) => {
              setCurrentMaximumRange(parseInt(value));}
            }
          />
          <SelectDropdown
              data={categories}
